...
Скрипт останавливает запущенный `mitmdump`, убирает алиас `agys` из профилей
шелла и удаляет папку установки.

### Как проходит запуск

При каждом вызове `agys` обёртка (`agy-tier.sh`):
...
| `AGY_MITM_PORT` | `8085` | стартовый порт локального mitmproxy |
| `AGY_KEEP_MITM` | `0` | `1` — не гасить поднятый mitmdump при выходе |

Порт выбирается при старте: если его уже занимает **наш** mitmdump (запущенный с
`tier-fix.py`), он переиспользуется; если порт занят **любым другим** процессом,
берётся следующий свободный — agy никогда не пойдёт через неизвестный прокси.
Проверяется до 50 портов от стартового.
...
`agys` сам гасит поднятый им прокси при выходе. Если процесс всё же завис:

```bash
kill "$(cat ~/.local/share/agy-tier-fix/mitm-tier.pid)"   # после install.sh
pkill -f 'mitmdump -s .*tier-fix.py'                      # или, независимо от пути
```
